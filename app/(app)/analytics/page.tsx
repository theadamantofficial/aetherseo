...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[var(--site-border)] bg-[var(--site-surface)] p-5">
        <div>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {translatedDashboard.cards.map((card) => (
          <article key={card.label} className="rounded-xl border border-[var(--site-border)] bg-[var(--site-surface)] p-5">
      ...
      <section className="rounded-xl border border-[var(--site-border)] bg-[var(--site-surface)] p-5">
        ...
        <div className="mt-5 h-64 rounded-lg border border-[var(--site-border)] bg-[var(--site-surface-soft)] p-4">
          {dashboard ? (
            <div className="grid h-full gap-4 md:grid-cols-3">
              <div className="rounded-xl border border-[var(--site-border)] p-4">
                ...
                <p className="mt-3 text-xl font-semibold">{recentActionCount}</p>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <article className="rounded-xl border border-[var(--site-border)] bg-[var(--site-surface)] p-5 lg:col-span-2">
          <h3 className="text-lg font-semibold">{ui.latestSignals}</h3>
          <div className="mt-5 text-sm">
            {translatedDashboard.health.length ? (
              <div className="divide-y divide-[var(--site-border)]">
                {translatedDashboard.health.map((row) => (
                  <div key={row.label} className="grid grid-cols-3 py-2.5">
                    <span>{row.label}</span>
                    <span>{row.value}</span>
        ...
        <article className="rounded-xl border border-[var(--site-border)] bg-[var(--site-surface)] p-5">
          <h3 className="text-lg font-semibold">{ui.actionVolume}</h3>
          <div className="mx-auto mt-6 flex h-44 w-44 items-center justify-center rounded-full border-[14px] border-[#634fff]">
            <div className="text-center">
              <p className="text-xl font-semibold">{translatedDashboard.usage?.current ?? 0}</p>
