  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-[1.3fr,0.7fr]">
        ...
        <article className="rounded-xl border border-[var(--site-border)] bg-[var(--site-surface)] p-5">
          <h3 className="text-lg font-semibold">{ui.subscriptionStatus}</h3>
          <div className="mt-4 space-y-3 text-sm">
            {[
              ...
              <div key={line} className="rounded-lg border border-[var(--site-border)] bg-[var(--site-surface-soft)] p-4">
                {line}
              </div>
            ))}
          </div>
      ...
      <section>
        <h3 className="text-center text-xl font-semibold">{ui.upgradeTitle}</h3>
        ...
        <div className="mt-6 grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(min(100%,16rem),1fr))]">
          {planCards.map((plan) => {
            ...
            return (
              <article
                key={plan.id}
                className={`flex min-w-0 h-full flex-col rounded-xl border p-5 bg-[var(--site-surface)] ${
                  isCurrent ? "border-[var(--site-primary)]" : "border-[var(--site-border)]"
                }`}
              >
                <div className="flex items-start justify-between gap-3">
                  <h4 className="text-lg font-semibold">{plan.title}</h4>
                  {isCurrent ? (
                    <span className="text-xs font-medium uppercase tracking-wider text-[var(--site-muted)]">
                      {ui.active}
                    </span>
                  ) : null}
                </div>
                ...
                <p className="mt-4 flex-1 text-sm leading-6 site-muted">
                  {ui.planBodies[plan.bodyKey]}
                </p>
                <Link
                  href={plan.id === "free" ? "/auth?plan=free" : `/choose-plan?tier=${plan.id}`}
                  className="mt-8 inline-flex w-full items-center justify-center rounded-lg px-4 py-3 text-sm font-semibold site-button-primary"
                >
