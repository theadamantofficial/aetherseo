...
export default function HistoryPage() {
  ...
  return (
    <div className="space-y-6">
          ...
          {(["all", "blog", "audit", "other"] as FilterKey[]).map((tab) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm ${
                filter === tab ? "site-button-primary" : "site-panel-soft"
              }`}
            >
              {ui.tabs[tab]}
            </button>
      ...
      {error ? <div className="site-panel rounded-2xl border p-4 text-sm">{ui.loadError}</div> : null}

      <section
        className="site-panel site-animate-rise overflow-x-auto rounded-2xl border"
        style={{ ["--site-delay" as string]: "80ms" }}
      >
        <table className="w-full min-w-[760px] text-left">
          <thead className="border-b border-[var(--site-border)] text-xs uppercase tracking-[0.14em] text-[var(--site-muted)]">
            <tr>
              <th className="p-4">{ui.table.name}</th>
              <th className="p-4">{ui.table.type}</th>
              <th className="p-4">{ui.table.date}</th>
              <th className="p-4">{ui.table.status}</th>
            </tr>
          </thead>
          <tbody>
            {filteredItems.length ? (
              filteredItems.map((item) => (
                <tr
                  ...
                  className="border-b border-[rgba(255,255,255,0.05)] text-sm"
                >
