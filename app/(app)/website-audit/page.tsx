...
export default function WebsiteAuditPage() {
  ...
  return (
    <div className="space-y-6">
      <section className="site-panel-hero site-animate-rise rounded-2xl border p-8 text-center">
        <p className="site-chip mx-auto inline-flex rounded-full border px-3 py-1 text-xs uppercase tracking-[0.18em]">
          {ui.badge}
        </p>
        <h1 className="text-5xl font-semibold">{ui.title}</h1>
        <p className="site-muted mt-3 text-sm">{ui.body}</p>
      </section>

      <section
        className="site-panel site-animate-rise flex flex-col gap-3 rounded-2xl border p-4 sm:flex-row"
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <article
          className="site-panel site-animate-rise rounded-2xl border p-8"
        ...
        >
          <div className="mx-auto flex h-64 w-64 items-center justify-center rounded-full border-[16px] border-[#7d6cff]">
            <div className="text-center">
              <p className="text-7xl font-semibold">{auditRun ? auditRun.score : "--"}</p>
              <p className="site-muted text-sm uppercase tracking-[0.2em]">{ui.score}</p>
            </div>
          </div>
          <p className="site-muted mx-auto mt-6 max-w-sm text-center text-sm">
            {auditRun?.summary ?? ui.scoreEmpty}
          </p>
        </article>

        <article
          className="site-panel site-animate-rise rounded-2xl border p-6"
          style={{ ["--site-delay" as string]: "160ms" }}
        >
          <h2 className="text-3xl font-semibold">{ui.breakdown}</h2>
          <div className="mt-5 space-y-3">
            {auditRun ? (
              auditRun.issues.map((item) => (
                <div key={item.title} className="site-panel-soft rounded-xl border p-4">
              ...
              <div className="site-panel-soft rounded-xl border p-4">
      ...
      <section className="space-y-3">
        <h3 className="text-2xl font-semibold">{ui.recommendations}</h3>
        <div className="grid gap-4 md:grid-cols-3">
          {auditRun ? (
            auditRun.recommendations.map((title) => (
              <article
                key={title}
                className="site-panel site-hover-lift site-animate-rise rounded-2xl border p-6"
            ...
            <article
              className="site-panel site-animate-rise rounded-2xl border p-6 md:col-span-3"
              style={{ ["--site-delay" as string]: "200ms" }}
            >
